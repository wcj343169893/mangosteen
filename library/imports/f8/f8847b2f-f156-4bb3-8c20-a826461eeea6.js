	...
	updateSort: function updateSort() {
		...
		if (newNode) {
			...
			this.scheduleOnce(function () {
				...
				var moveAction = cc.sequence(cc.spawn(cc.scaleTo(0.1, 0.8, 1.2), cc.rotateTo(0.1, 61.0), cc.moveTo(0.1, newPosFirst)), cc.spawn(cc.scaleTo(0.2, 1, 1), cc.rotateTo(0.2, 0), cc.moveTo(0.2, pos2)), cc.delayTime(0.5), cc.spawn(cc.scaleTo(0.1, this.initScale), cc.moveTo(0.1, newPos))
				// 以1/2的速度慢放动画，并重复5次
				).speed(2); //.repeat(5);
				//初始设置类似新牌位置
